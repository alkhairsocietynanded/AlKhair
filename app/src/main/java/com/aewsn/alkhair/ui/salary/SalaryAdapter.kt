...
import com.aewsn.alkhair.databinding.ItemSalaryBinding
...
) : ListAdapter<SalaryModel, SalaryAdapter.SalaryViewHolder>(
    // ✅ Use Background Thread for DiffUtil to prevent UI freeze
    androidx.recyclerview.widget.AsyncDifferConfig.Builder(DiffCallback())
        .setBackgroundThreadExecutor(java.util.concurrent.Executors.newSingleThreadExecutor())
        .build()
) {
    ...
    inner class SalaryViewHolder(private val binding: ItemSalaryBinding) :
        ...
        fun bind(salary: SalaryModel) {
            ...
            binding.apply {
                ...
                tvStaffName.text = salary.staffName.ifBlank { "Unknown Staff" }
                tvMonthYear.text = DateUtils.formatSalaryMonth(salary.salaryDate)
                ...
                tvPaymentStatus.backgroundTintList = ColorStateList.valueOf(statusColor)
